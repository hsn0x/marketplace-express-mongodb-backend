...
export default {
    getById: async (req, res) => {
        ...
        const data = await votesQueries.findByIdQuery(id)
        if (data) {
    ...
    getBySlug: async (req, res) => {
        ...
        const data = await votesQueries.findOneQuery({ slug })
        if (data) {
            res.status(200).json(data)
        } else {
            res.status(404).json({
    ...
    getAll: async (req, res) => {
        ...
        const data = await votesQueries.findAllQuery({}, [], [], params)
        if (data) {
    ...
    getAllByFilters: async (req, res) => {
        ...
        const data = await votesQueries.findAllQuery(filter, [], [], params)
        if (data) {
            return res.status(200).json(data)
        } else {
            return res.status(404).json({ message: "No Data" })
    ...
    create: async (req, res, next) => {
        ...
        const isValid = VoteValidation.validateCreate(data)
    ...
    update: async (req, res) => {
        ...
        const isValid = VoteValidation.validateUpdate(data)
        if (!isValid.valid) {
            return res.status(400).json({
                message: "Invalid record data",
                errors: isValid.errors,
        ...
        const updatedRecord = await votesQueries.updateOneQuery(
            { _id: id },
            data
        )
        if (updatedRecord) {
    ...
    remove: async (req, res) => {
        const id = req.params.id
        await votesQueries.deleteOneQuery({ _id: id })
